...
class DataProcessor:
    ...
    def preprocess_for_training(
        self, df: Optional[pd.DataFrame] = None, target_column: str = "actual_clv"
    ...
    def temporal_split(
        ...
        logger.info(
            f"Performing temporal split: train={train_ratio}, val={val_ratio}, test={test_ratio}"
        )
        ...
        if date_column in df.columns:
            ...
            logger.info(
                f"Val date range: {val_df[date_column].min()} to {val_df[date_column].max()}"
            )
            logger.info(
