...
class AdvancedFeatureEngineer:
    ...
    def _register_feature(self, name: str, category: FeatureCategory, description: str) -> None:
    ...
    def calculate_rfm_scores(self, df: pd.DataFrame, method: str = "quantile") -> pd.DataFrame:
        """
        ...
        if "days_since_last_purchase" in df.columns:
            ...
            self._register_feature(
                "recency_normalized", FeatureCategory.RFM, "Normalized recency (0-1)"
            )
        ...
        if "total_orders" in df.columns:
            ...
            self._register_feature(
                "frequency_score", FeatureCategory.RFM, "RFM frequency score (1-5)"
            )
            self._register_feature(
                "frequency_log", FeatureCategory.RFM, "Log-transformed order count"
            )
            self._register_feature(
                "frequency_normalized", FeatureCategory.RFM, "Normalized frequency (0-1)"
            )
        ...
        if "total_spent" in df.columns:
            ...
            self._register_feature(
                "monetary_score", FeatureCategory.RFM, "RFM monetary score (1-5)"
            )
            self._register_feature(
                "monetary_log", FeatureCategory.RFM, "Log-transformed total spent"
            )
            self._register_feature(
                "monetary_normalized", FeatureCategory.RFM, "Normalized monetary (0-1)"
            )
        ...
        if all(col in df.columns for col in required_scores):
            ...
            self._register_feature(
                "rfm_weighted", FeatureCategory.RFM, "Weighted RFM (CLV-optimized)"
            )
    ...
    def create_behavioral_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "total_orders" in df.columns and "days_since_first_purchase" in df.columns:
            ...
            if "days_since_last_purchase" in df.columns:
                ...
                df["recency_ratio"] = np.where(tenure > 0, 1 - (recent_activity / tenure), 0)

            self._register_feature(
                "purchase_velocity", FeatureCategory.BEHAVIORAL, "Orders per month"
            )
            self._register_feature("orders_per_week", FeatureCategory.BEHAVIORAL, "Orders per week")
            self._register_feature(
                "recency_ratio", FeatureCategory.BEHAVIORAL, "Activity recency ratio"
            )
        ...
        if "total_orders" in df.columns and "days_since_first_purchase" in df.columns:
            ...
            self._register_feature(
                "purchase_regularity", FeatureCategory.BEHAVIORAL, "Purchase regularity score"
            )
        ...
        if "total_spent" in df.columns and "total_orders" in df.columns:
            ...
            if "days_since_first_purchase" in df.columns:
                ...
                self._register_feature(
                    "spending_velocity", FeatureCategory.BEHAVIORAL, "Spending per month"
                )
            ...
            if "avg_order_value" in df.columns:
                ...
                self._register_feature(
                    "aov_deviation", FeatureCategory.BEHAVIORAL, "AOV deviation from median"
                )
        ...
        if "email_engagement_rate" in df.columns:
            ...
            self._register_feature(
                "email_engagement_bucket", FeatureCategory.BEHAVIORAL, "Email engagement tier"
            )
        ...
        if "return_rate" in df.columns:
            ...
            df["return_risk"] = np.where(
                df["return_rate"] > 0.3, 2, np.where(df["return_rate"] > 0.15, 1, 0)
            )

            self._register_feature(
                "is_returner", FeatureCategory.BEHAVIORAL, "Has high return rate"
            )
            self._register_feature(
                "return_risk", FeatureCategory.BEHAVIORAL, "Return risk level (0-2)"
            )

        # === COMPOSITE ENGAGEMENT SCORE ===
        if "email_engagement_rate" in df.columns and "return_rate" in df.columns:
            df["engagement_score"] = (
                df["email_engagement_rate"] * 0.6 + (1 - df["return_rate"]) * 0.4
            )
            self._register_feature(
                "engagement_score", FeatureCategory.BEHAVIORAL, "Composite engagement (0-1)"
            )
        ...
        if "days_since_first_purchase" in df.columns:
            ...
            self._register_feature(
                "tenure_months", FeatureCategory.BEHAVIORAL, "Customer tenure in months"
            )
            self._register_feature(
                "tenure_bucket", FeatureCategory.BEHAVIORAL, "Tenure bucket (1-6)"
            )
            self._register_feature(
                "customer_maturity", FeatureCategory.BEHAVIORAL, "Maturity score (0-1)"
            )
        ...
        if "num_categories" in df.columns:
            ...
            self._register_feature(
                "is_multi_category", FeatureCategory.BEHAVIORAL, "Shops 3+ categories"
            )
            self._register_feature(
                "category_diversity", FeatureCategory.BEHAVIORAL, "Category diversity score"
            )
        ...
        if "days_since_last_purchase" in df.columns:
            ...
            df["churn_risk_score"] = np.where(
                df["days_since_last_purchase"] > mean_recency + 2 * std_recency,
                3,
                np.where(
            ...
            self._register_feature(
                "churn_risk_score", FeatureCategory.BEHAVIORAL, "Churn risk (0-3)"
            )
    ...
    def create_temporal_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "first_purchase_date" in df.columns:
            try:
                ...
                self._register_feature(
                    "first_purchase_dow", FeatureCategory.TEMPORAL, "First purchase day of week"
                )
                self._register_feature(
                    "first_purchase_month", FeatureCategory.TEMPORAL, "First purchase month"
                )
                self._register_feature(
                    "first_purchase_quarter", FeatureCategory.TEMPORAL, "First purchase quarter"
                )
                self._register_feature(
                    "first_purchase_is_weekend",
                    FeatureCategory.TEMPORAL,
                    "First purchase on weekend",
                )
                self._register_feature(
                    "first_purchase_season", FeatureCategory.TEMPORAL, "First purchase season"
                )
        ...
        if "last_purchase_date" in df.columns:
            try:
                ...
                self._register_feature(
                    "last_purchase_dow", FeatureCategory.TEMPORAL, "Last purchase day of week"
                )
                self._register_feature(
                    "last_purchase_month", FeatureCategory.TEMPORAL, "Last purchase month"
                )
        ...
        if "days_since_last_purchase" in df.columns:
            ...
            self._register_feature(
                "purchased_last_7d", FeatureCategory.TEMPORAL, "Purchased in last 7 days"
            )
            self._register_feature(
                "purchased_last_30d", FeatureCategory.TEMPORAL, "Purchased in last 30 days"
            )
            self._register_feature(
                "purchased_last_90d", FeatureCategory.TEMPORAL, "Purchased in last 90 days"
            )
            self._register_feature(
                "inactive_180d", FeatureCategory.TEMPORAL, "Inactive for 180+ days"
            )
    ...
    def create_statistical_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in zscore_columns:
            if col in df.columns:
                ...
                if std > 0:
                    ...
                    self._register_feature(
                        f"{col}_zscore", FeatureCategory.STATISTICAL, f"Z-score of {col}"
                    )
                    self._register_feature(
                        f"{col}_percentile",
                    ...
                    self._register_feature(
                        f"{col}_is_outlier", FeatureCategory.STATISTICAL, f"Outlier flag for {col}"
                    )
        ...
        if outlier_cols:
            ...
            self._register_feature(
                "outlier_score", FeatureCategory.STATISTICAL, "Number of outlier flags"
            )
    ...
    def create_acquisition_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "acquisition_source" in df.columns:
            ...
            df["is_paid_acquisition"] = (
                df["acquisition_source"].isin(["Meta Ads", "Google Ads"]).astype(int)
            )
            df["is_organic"] = (
                df["acquisition_source"].isin(["Organic", "Direct", "Referral"]).astype(int)
            )
            df["is_referral"] = (df["acquisition_source"] == "Referral").astype(int)

            self._register_feature(
                "source_quality_score", FeatureCategory.ACQUISITION, "Source quality (0-1)"
            )
            self._register_feature(
                "is_paid_acquisition", FeatureCategory.ACQUISITION, "Acquired via paid channel"
            )
            self._register_feature(
                "is_organic", FeatureCategory.ACQUISITION, "Acquired organically"
            )
            self._register_feature(
                "is_referral", FeatureCategory.ACQUISITION, "Acquired via referral"
            )
        ...
        if "campaign_type" in df.columns:
            ...
            self._register_feature(
                "campaign_effectiveness", FeatureCategory.ACQUISITION, "Campaign type effectiveness"
            )
            self._register_feature(
                "is_retargeted", FeatureCategory.ACQUISITION, "Acquired via retargeting"
            )
        ...
        if "acquisition_cost" in df.columns:
            ...
            if "total_spent" in df.columns:
                ...
                self._register_feature(
                    "cac_efficiency", FeatureCategory.ACQUISITION, "Revenue / CAC ratio"
                )
                self._register_feature(
                    "cac_roi", FeatureCategory.ACQUISITION, "CAC return on investment"
                )
                self._register_feature(
                    "cac_paid_back", FeatureCategory.ACQUISITION, "CAC recovered flag"
                )
    ...
    def create_interaction_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "total_spent" in df.columns and "total_orders" in df.columns:
            ...
            if "avg_order_value" in df.columns:
                median_aov = df["avg_order_value"].median()
                df["value_frequency_score"] = (df["avg_order_value"] / median_aov) * np.log1p(
                    df["total_orders"]
                )
                self._register_feature(
                    "value_frequency_score",
                    FeatureCategory.INTERACTION,
                    "Value x frequency composite",
                )
        ...
        if "engagement_score" in df.columns and "monetary_normalized" in df.columns:
            ...
            self._register_feature(
                "engaged_value_score", FeatureCategory.INTERACTION, "Engagement x monetary value"
            )
        ...
        if "recency_normalized" in df.columns and "frequency_normalized" in df.columns:
            ...
            self._register_feature(
                "rf_score", FeatureCategory.INTERACTION, "Recency x frequency product"
            )
        ...
        if "customer_maturity" in df.columns and "monetary_normalized" in df.columns:
            ...
            self._register_feature(
                "mature_value_score", FeatureCategory.INTERACTION, "Maturity x value score"
            )
        ...
        if "source_quality_score" in df.columns and "monetary_normalized" in df.columns:
            ...
            self._register_feature(
                "source_value_score", FeatureCategory.INTERACTION, "Source quality x value"
            )
        ...
        if len(available) >= 2:
            ...
            self._register_feature(
                "clv_predictor_composite",
                FeatureCategory.INTERACTION,
                "CLV predictor composite score",
            )
        ...
        if "acquisition_source" in df.columns and "num_categories" in df.columns:
            ...
            self._register_feature(
                "source_category_interaction",
                FeatureCategory.INTERACTION,
                "Acquisition source × category count",
            )
        ...
        if "campaign_type" in df.columns and "total_spent" in df.columns:
            ...
            df["campaign_value_interaction"] = df["campaign_numeric"] * (
                df["total_spent"] / median_spent
            )
            self._register_feature(
                "campaign_value_interaction",
                FeatureCategory.INTERACTION,
                "Campaign type × normalized value",
            )

        # === TIME OF YEAR × BEHAVIOR INTERACTIONS ===
        if "first_purchase_quarter" in df.columns and "total_orders" in df.columns:
            df["quarter_frequency_interaction"] = df["first_purchase_quarter"] * np.log1p(
                df["total_orders"]
            )
            self._register_feature(
                "quarter_frequency_interaction",
                FeatureCategory.INTERACTION,
                "First purchase quarter × frequency",
            )

        if "first_purchase_season" in df.columns and "avg_order_value" in df.columns:
            median_aov = df["avg_order_value"].median()
            df["season_value_interaction"] = df["first_purchase_season"] * (
                df["avg_order_value"] / median_aov
            )
            self._register_feature(
                "season_value_interaction", FeatureCategory.INTERACTION, "Season × AOV"
            )
        ...
        if "email_engagement_rate" in df.columns and "days_since_last_purchase" in df.columns:
            ...
            max_recency = df["days_since_last_purchase"].max()
            recency_inverted = (
                1 - (df["days_since_last_purchase"] / max_recency) if max_recency > 0 else 0
            )
            df["engagement_recency_interaction"] = df["email_engagement_rate"] * recency_inverted
            self._register_feature(
                "engagement_recency_interaction",
                FeatureCategory.INTERACTION,
    ...
    def create_holiday_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "first_purchase_date" in df.columns:
            try:
                ...
                major_cols = [
                    f"days_to_{h}" for h in major_holidays if f"days_to_{h}" in df.columns
                ]
                if major_cols:
                    df["days_to_major_holiday"] = df[major_cols].min(axis=1)
                    self._register_feature(
                        "days_to_major_holiday",
                        FeatureCategory.TEMPORAL,
                        "Days to nearest major shopping holiday",
                    )
                ...
                self._register_feature(
                    "is_peak_season", FeatureCategory.TEMPORAL, "First purchase in peak season"
                )
                ...
                if near_cols:
                    ...
                    self._register_feature(
                        "near_any_holiday",
                        FeatureCategory.TEMPORAL,
                        "First purchase near any holiday",
                    )
    ...
    def fit_transform(
        ...
        logger.info(
            f"Feature engineering complete. Created {len(self._feature_columns)} new features."
        )
        return result
    ...
    def get_feature_names(self, category: Optional[FeatureCategory] = None) -> List[str]:
        ...
        return [name for name, meta in self._feature_metadata.items() if meta.category == category]
