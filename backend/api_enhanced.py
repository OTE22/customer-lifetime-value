...
from fastapi import FastAPI, Query, Path, Body, Depends, HTTPException, Request
...
async def get_customer(
    customer_id: str = Path(..., min_length=1, max_length=50),
    predictor = Depends(get_predictor),
    cache: CacheManager = Depends(get_cache_manager)
):
...
async def train_model(
    ...
    try:
        ...
        if predictor.model_trained and not force:
            return {
                "status": "skipped",
...
async def get_dashboard_summary(
    ...
    try:
        ...
        # Top customers
        top_customers = df.nlargest(5, clv_col).to_dict('records') if clv_col in df.columns else []
