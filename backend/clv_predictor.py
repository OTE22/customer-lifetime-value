...
class CLVPredictor:
    ...
    def __init__(self, model_dir: str = "models", data_path: Optional[str] = None):
        """
    ...
    def train(
        self, data_path: Optional[str] = None, test_size: float = 0.2, save_models: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        feature_importance = self.model_trainer.get_feature_importance(
            "random_forest", self.feature_columns
        )
    ...
    def segment_customers(self, df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
        """
        ...
        logger.info(
            f"Segmented customers - High: {len(segments['High-CLV'])}, "
    ...
    def predict_with_confidence(
        self, customer_data: Dict[str, Any], confidence_level: float = 0.95
    ) -> Dict[str, Any]:
        """
        ...
        return {
            **base_result,
            ...
            "model_predictions": {
                ...
                "spread": (
                    round(float(max(model_predictions) - min(model_predictions)), 2)
                    if len(model_predictions) >= 2
                    else None
                ),
            },
