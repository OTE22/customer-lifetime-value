...
class ProductionEnsemble:
    ...
    def train(self, X: pd.DataFrame, y: pd.Series, cv_folds: int = 5) -> Dict[str, Any]:
        ...
        logger.info(
            f"Ensemble trained: MAE=${self.ensemble_metrics.mae:.2f}, R²={self.ensemble_metrics.r2:.3f}"
        )
    ...
    def _calculate_optimal_weights(
        self, y_true: pd.Series, predictions: Dict[str, np.ndarray]
    ) -> None:
    ...
    def _ensemble_predict_internal(self, predictions: Dict[str, np.ndarray]) -> np.ndarray:
    ...
    def predict(
        self, X: pd.DataFrame, return_individual: bool = False
    ...
    def get_feature_importance(self) -> pd.DataFrame:
        ...
        importance = pd.DataFrame(
            {"feature": importance_sum.index, "importance": importance_sum.values}
        ).sort_values("importance", ascending=False)
        importance["rank"] = range(1, len(importance) + 1)
...
class CLVModelTrainer(ProductionEnsemble):
    ...
    def get_feature_importance(
        self, model_name: str = None, feature_names: List[str] = None
    ) -> pd.DataFrame:
