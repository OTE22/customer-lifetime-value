...
class CustomerFeatures(BaseSchema):
    ...
    days_since_first_purchase: int = Field(..., ge=0, le=3650, description="Days since first purchase")
    days_since_last_purchase: int = Field(..., ge=0, le=3650, description="Days since last purchase")
    ...
    acquisition_cost: float = Field(default=0, ge=0, le=10000, description="Customer acquisition cost")
    email_engagement_rate: float = Field(default=0.0, ge=0, le=1, description="Email engagement rate (0-1)")
    ...
    def validate_date_consistency(self):
        if self.days_since_last_purchase > self.days_since_first_purchase:
            raise ValueError('days_since_last_purchase cannot be greater than days_since_first_purchase')
        return self
