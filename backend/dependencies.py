...
def get_predictor(container: ServiceContainer = Depends(get_service_container)):
    """Dependency to inject CLV predictor."""
    return container.predictor


def get_data_processor(container: ServiceContainer = Depends(get_service_container)):
    """Dependency to inject data processor."""
    return container.data_processor


def get_feature_engineer(container: ServiceContainer = Depends(get_service_container)):
    """Dependency to inject feature engineer."""
    return container.feature_engineer


def get_meta_ads(container: ServiceContainer = Depends(get_service_container)):
...
class HealthChecker:
    ...
    async def check_all(self) -> dict:
        ...
        try:
            ...
            components.append(
                {"name": "data", "status": data_status, "latency_ms": round(latency, 2)}
            )
        except Exception as e:
            components.append({"name": "data", "status": "unhealthy", "message": str(e)})
        ...
        try:
            ...
            components.append(
                {"name": "model", "status": model_status, "latency_ms": round(latency, 2)}
            )
        except Exception as e:
            components.append({"name": "model", "status": "unhealthy", "message": str(e)})
            overall_status = "unhealthy"
        ...
        try:
            ...
            components.append({"name": "cache", "status": cache_status, "stats": cache_stats})
        except Exception as e:
            components.append({"name": "cache", "status": "unhealthy", "message": str(e)})
