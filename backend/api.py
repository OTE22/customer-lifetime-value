...
from .exceptions import CLVException, CustomerNotFoundError, ModelNotTrainedError
from .logging_config import LoggerFactory, LogMetrics, get_logger
...
@app.get(
    "/api/health", response_model=HealthCheck, tags=["Health"], summary="Health check endpoint"
)
async def health_check(
    checker=Depends(get_health_checker), config: CLVConfig = Depends(get_config_dependency)
):
...
@app.get("/api/status", tags=["Health"], summary="Detailed status information")
...
async def batch_predict(request: BatchPredictionRequest, predictor=Depends(get_predictor)):
...
async def get_segments(predictor=Depends(get_predictor)):
    ...
    try:
        ...
        for seg in segments.values():
            seg.percentage = (
                round((seg.count / total_customers) * 100, 1) if total_customers > 0 else 0
            )

        return SegmentAnalysis(
            segments=segments, total_customers=total_customers, total_value=round(total_value, 2)
        )
...
async def get_model_metrics(
    predictor=Depends(get_predictor), config: CLVConfig = Depends(get_config_dependency)
):
    ...
    try:
        ...
        return ModelPerformance(
            ...
            metrics=ModelMetrics(
                mae=metrics.get("mae", 0), rmse=metrics.get("rmse", 0), r2=metrics.get("r2", 0)
            ),
            feature_importance=[
                FeatureImportance(
                    feature=row["feature"], importance=round(row["importance"], 4), rank=i + 1
                )
...
@app.post("/api/model/train", tags=["Model"], summary="Train or retrain the model")
...
@app.get("/api/meta-ads/audiences", tags=["Meta Ads"], summary="Get audience segments for Meta Ads")
async def get_audiences(meta_ads=Depends(get_meta_ads), predictor=Depends(get_predictor)):
        ...
        return BudgetAllocationResponse(
            ...
            allocation={
                seg: BudgetAllocation(
                    ...
                    optimal_cac=round(
                        data.get(
                            "optimal_cac", data["budget"] / max(data["expected_acquisitions"], 1)
                        ),
                        2,
                    ),
                )
...
async def get_dashboard_summary(
    predictor=Depends(get_predictor), cache: CacheManager = Depends(get_cache_manager)
):
    ...
    try:
        ...
        # Acquisition sources
        sources = (
            df["acquisition_source"].value_counts().to_dict()
...
@app.get(
    "/api/cache/stats", response_model=CacheStats, tags=["Admin"], summary="Get cache statistics"
)
...
@app.post("/api/cache/clear", tags=["Admin"], summary="Clear the cache")
...
async def clv_exception_handler(request: Request, exc: CLVException):
    ...
    return JSONResponse(status_code=getattr(exc, "status_code", 500), content=exc.to_dict())
...
async def http_exception_handler(request: Request, exc: HTTPException):
    ...
    return JSONResponse(
        ...
        content={"error": True, "error_code": exc.status_code * 10, "message": exc.detail},
    )
